...
void main()
{
    ...
    vec3 R = N;     // Approximation of reflection vector
    vec3 V = R;     // Approximation of view vector
    
    const uint SAMPLE_NR = 1024u;
    float totalWeight = 0.0;
    vec3 prefilteredColor = vec3(0.0);
    
    for (uint i = 0u; i < SAMPLE_NR; ++i)
    {
        vec2 Xi = Hammersley(i, SAMPLE_NR);
